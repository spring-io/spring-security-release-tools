...
public abstract class CreateGitHubReleaseTask extends DefaultTask {
	...
	public CreateGitHubReleaseTask() {
		...
		getCreateRelease().convention(false);
	}
	...
	public static TaskProvider<CreateGitHubReleaseTask> register(Project project) {
		...
		return project.getTasks().register(TASK_NAME, CreateGitHubReleaseTask.class, (task) -> {
			...
			var createRelease = "true".equals(project.findProperty(CREATE_RELEASE_PROPERTY));
			task.getCreateRelease().set(createRelease);
			...
			if (project.hasProperty("branch")) {
				task.getBranch().set((String) project.findProperty(BRANCH_PROPERTY));
			}
			var gitHubAccessToken = (String) project.findProperty(GITHUB_ACCESS_TOKEN_PROPERTY);
			task.getGitHubAccessToken().set(gitHubAccessToken);
